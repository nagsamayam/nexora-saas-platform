...
class SendTenantApprovedEmailJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        if (! Cache::add($idempotencyKey, 'processing', 86400)) {
            ...
            if ($status === 'completed' || $status === 'processing') {
                Log::info('Duplicate job execution skipped for SendTenantApprovedEmailJob', [
        ...
        try {
            ...
            if ($email !== '') {
                Mail::to($email)->send(new TenantApprovedMail(
                    userName: $name,
                    userEmail: $email,
            ...
            Log::info('Tenant approved email successfully processed and sent', [
                'event_id' => $this->eventId,
            ...
            Log::error('Failed to send tenant approved email', [
                'event_id' => $this->eventId,
                'tenant_id' => $this->aggregateId,
