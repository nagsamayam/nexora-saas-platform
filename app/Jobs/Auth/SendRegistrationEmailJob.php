    ...
    public function handle(): void
    {
        ...
        if (! Cache::add($idempotencyKey, 'processing', 86400)) {
            ...
            if ($status === 'completed' || $status === 'processing') {
                Log::info('Duplicate job execution skipped for SendRegistrationEmailJob', [
        ...
        try {
            ...
            $name = (string) ($this->payload['name'] ?? ($this->payload['first_name'] ?? 'User'));

            if ($email !== '') {
                Mail::to($email)->send(new WelcomeRegistrationMail($name, $email));
            }

            Cache::put($idempotencyKey, 'completed', 86400 * 7);

            Log::info('Registration email successfully processed and sent', [
                'event_id' => $this->eventId,
